...
function getNextDay (start) {
  return dayjs(start).add(1, 'day').startOf('day').valueOf()
}

function getNextHour (start) {
  return dayjs(start).add(1, 'hour').startOf('hour').valueOf()
}
...
function identifyLogFile (checkedFileName, fileVal, dateFormat, extension) {
  ...
  if (typeof dateFormat === 'string' && dateFormat.length > 0) {
    const d = dayjs(checkFileNameSegments[0], dateFormat)
...
function parseDate (formatStr, frequencySpec, parseStart = false) {
  ...
  try {
    return dayjs(parseStart ? frequencySpec.start : frequencySpec.next).format(
      formatStr
    )
  } catch (error) {
