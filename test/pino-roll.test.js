...
test("rotate file based on time and parse filename func", async ({
  ok,
  notOk,
  rejects,
}) => {
  const file = join(logFolder, "log");
  await sleep(100 - (Date.now() % 100));
  const fileFunc = () => `${file}-${dayjs().format("HH-mm-ss-SSS")}`;
...
test("removeOtherOldFiles()", async ({ ok, notOk }) => {
  ...
  notOk(
    files.includes(`${file0}.1`),
    "first run: fourth recent file is removed"
  );
  notOk(
    files.includes(`${file1}.1`),
    "first run: third recent file is removed"
  );
  ok(
    files.includes(`${file1}.2`),
    "first run: second recent file is not removed"
  );
  ok(
    files.includes(`${file2}.1`),
    "first run: most recent file is not removed"
  );
  ...
  notOk(
    files.includes(`${file0}.1`),
    "second run: fourth recent file is removed"
  );
  notOk(
    files.includes(`${file1}.1`),
    "second run: third recent file is removed"
  );
  ok(
    files.includes(`${file1}.2`),
    "second run: second recent file is not removed"
  );
  ok(
    files.includes(`${file2}.1`),
    "second run: most recent file is not removed"
  );
...
test("remove pre-existing log files when removing files based on count when limit.removeOtherLogFiles", async ({
  ...
  const prev = new Date(now.getTime() - 500);
   const prevFile = join(logFolder, `log.${dayjs(prev).format(dateFormat)}`);
   ...
   const file1 = join(
     logFolder,
     `log.${dayjs(now).format(dateFormat)}`
   );
   const file2 = join(
     logFolder,
     `log.${dayjs(now.getTime() + 1500).format(dateFormat)}`
   );
